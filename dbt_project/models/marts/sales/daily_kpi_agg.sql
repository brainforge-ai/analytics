...
with order_kpis as (
    select
        ...
        --product cost cogs
        NVL(sum(cogs_product_cost),0) as cogs_product_cost, -- FYI: This is being used above already
        NVL(sum(order_box_cost),0) as fulfillment_costs,
        NVL(sum(order_shipping_cost) + sum(order_pick_cost),0) as other_fees,
        NVL(sum(TOTAL_LINE_ITEMS_PRICE - TOTAL_DISCOUNTS + REFUND_AMOUNT), 0)
            - NVL(sum(cogs_product_cost),0)
            - NVL(sum(order_box_cost),0)
            - NVL(sum(platform_fee),0)
            - NVL(sum(order_shipping_cost) + sum(order_pick_cost),0) as gross_margin

        
    from {{ref('fact_orders')}}
    WHERE 
        IS_TIKTOK_SHOP = FALSE
        and app_source = 'Shopify'
    group by all
),
...
final as (
    select
        ...
        --product cost cogs
        ok.cogs_product_cost,
        ok.fulfillment_costs,
        ok.other_fees,
        ok.gross_margin
